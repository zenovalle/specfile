...
# More CVE
Patch5:         openjpeg2_CVE-2019-12973.patch
...
# XXX: PASE hacks include:
# - explicit zlib path (otherwise it tries to use the one from SC1)
# - explicit use of cmake (no cmake macro with PASE?)
# - we dont have doxygen yet
cmake -DCMAKE_BUILD_TYPE=RelWithDebInfo -DOPENJPEG_INSTALL_LIB_DIR=%{_lib} \
    ...
    -DBUILD_DOC=ON -DZLIB_ROOT=%{_prefix} \
...
%files
# Ugh, PASE cmake doesn't support versioned shared objects
# Commenting out versions at the end and devel unversioned because of it
%defattr(-, qsys, *none)
...
- Convert to PASE
- Bump
- De-version symbols because CMake on PASE doesn't (ugh)
- Don't use dev docs because no doxygen
